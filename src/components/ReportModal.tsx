...
const handleSubmit = async () => {
  ...
  try {
    ...
    const res = await fetch(`${API_URL}/api/posts/create`, {
      method: "POST",
      body: formData, // ✅ correct, do NOT set Content-Type manually
    });
  ...
  navigator.geolocation.getCurrentPosition(
    async (pos) => {
      ...
      try {
        ...
        const detected = data.display_name || `${latitude}, ${longitude}`;
...
>
  ...
  className="sr-only" // <-- screen-reader only, still clickable via label
  id="file-upload"
/>
